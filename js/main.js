...
function getTop4(intValue){
    ...
    try {
        ...
        for(let[city,coordinates,] of map_object.entries()){      
                ...
                .catch(error => console.log('Error caught,showing error: ', error));
                //I like this better but there really isn't an issue with the first three city's
                // .catch(error => window.alert("Whoops, we might have a problem"))           
        }
    }catch{   
        //let message = "Sorry,the map won't display"
...
function initMap(top4,prov) { 
    ...
    // Googgles error seems to overwrite the alerts 
    }catch(e){
        let message = "Sorry,the map won't display"
        alert(e.message)
    }
